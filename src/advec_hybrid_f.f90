!> \file advec_hybrid_f.f90
!
!  This is a faster version of the hybrid advection scheme in advec_hybrid.f90.
!  It is supposed to give identical results, except possibly for different rounding and other numerical effects.
!  The main changes are:
!   * all calculations are merged in the same function.
!     The parameter passing in the original turned out to happen via temporary arrays, hich was slow
!  * the smoothness criterion is tested without division
!  * fewer temporary arrays
!  
!  Further speed-up potential:
!  - try handling x, y, z in separate loops (possibly separate i,j but common k)
!  - make pfacex, pfacey, pfacez arrays again. making them scalars gave a small speed penalty,
!    probably due to writing the result in two places
! 
!  Fredrik Jansson, CWI 2018
! 
