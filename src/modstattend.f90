...
subroutine initstattend
    ...
    nsamples = itimeav/idtav
     if (myid==0) then
        ...
        call ncinfo(ncname(28,:),'thltendadv','theta_l advective tendency','K/s','tt')
        call ncinfo(ncname(29,:),'thltenddif','theta_l diffusive tendency','K/s','tt')
        call ncinfo(ncname(30,:),'thltendrad','theta_l radiative tendency','K/s','tt')
        call ncinfo(ncname(31,:),'thltendmicro','theta_l microphysical tendency','K/s','tt')
        call ncinfo(ncname(32,:),'thltendls','theta_l large scale tendency','K/s','tt')
        call ncinfo(ncname(33,:),'thltendtop','theta_l  top boundary tendency','K/s','tt')
        call ncinfo(ncname(34,:),'thltendaddon','theta_l in addons tendency','K/s','tt')
        call ncinfo(ncname(35,:),'thltendtot','theta_l total tendency','K/s','tt')
...
!> Write the statistics to file
  subroutine writestattend
    ...
    open(ifoutput,file='utend.'//cexpnr,position='append')
     write(ifoutput,'(//A,/A,I4,A,I2,A,I2,A)') &
      ...
      write (ifoutput,'(2A/A/A/2A/2A/2A/2A)') &
           ...
           ,'#----------------------------------------------------------' &
           ,'----------------------------------------------------------'
      write(ifoutput,'(I4,F10.2,F7.1,9E13.5)') &
           (k, &
            zf       (k), &
            presf    (k)/100., &
            upmn   (k,tend_adv), &
    ...
    open(ifoutput,file='thltend.'//cexpnr,position='append')
     write(ifoutput,'(//A,/A,I4,A,I2,A,I2,A)') &
         '#--------------------------------------------------------'      &
         ,'#--- AVERAGING TIMESTEP --- '      &
         ,nhrs,':',nminut,':',nsecs      &
      ...
      write (ifoutput,'(2A/A/A/2A/2A/2A/2A)') &
           ...
           ,'#----------------------------------------------------------' &
           ,'----------------------------------------------------------'
      write(ifoutput,'(I4,F10.2,F7.1,8E13.5)') &
           (k, &
            zf       (k), &
            presf    (k)/100., &
            thlpmn   (k,tend_adv), &
    ...
    open(ifoutput,file='qttend.'//cexpnr,position='append')
     write(ifoutput,'(//A,/A,I4,A,I2,A,I2,A)') &
      ...
      write (ifoutput,'(2A/A/A/2A/2A/2A/2A)') &
           ...
           ,'#----------------------------------------------------------' &
           ,'----------------------------------------------------------'
      write(ifoutput,'(I4,F10.2,F7.1,7E13.5)') &
           (k, &
            zf       (k), &
            presf    (k)/100., &
            qtpmn   (k,tend_adv), &
