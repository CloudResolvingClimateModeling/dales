        ...
        do i=2,i1
          if (ldelta .or. (dthvdz(i,j,k)<=0)) then
            ...
            ekh(i,j,k) = max(ekh(i,j,k),ekmin)
             ...
             ! faster calculation: evaluate sqrt only if the second term is actually smaller
             zlt(i,j,k) = delta(k)
...
!-----------------------------------------------------------------|
  ...
  use modglobal,   only : i1,j1,kmax,delta,dx,dy,dxi,dyi,dzf,dzh,grav, cu, cv,rk3step,rtimee,ntimee,deltai
...
!            sbshr(2:i1,2:j1,1)+sbbuo(2:i1,2:j1,1)+sbdiss(2:i1,2:j1,1)  
    ! e120i2 = 1/(2*e120(i,j,k))
    e12p(i,j,k) =   e12p(i,j,k) &
                  + ( ekm(i,j,k) * tdef2  &
                      - ekh(i,j,k)*grav/thvf(k)*dthvdz(i,j,k) ) / (2*e120(i,j,k)) &
                  - (ce1 + ce2*zlt(i,j,k)*deltai(k)) * e120(i,j,k)**2 /(2.*zlt(i,j,k))
