...
!  real                                      :: rhobfinvk                      
  ...
  integer :: i,j,k
  ! dirx $ assume_aligned p:64, q:64
  ! dirx $ assume(mod( i1+ih - (2-ih) + 1, 8) == 0)
  ! dirx $ assume(mod( j1+jh - (2-jh) + 1, 8) == 0)
  ! dirx $ assume(mod(size(p,1), 8) == 0)
  ! dirx $ assume(mod(size(p,2), 8) == 0)
  ! dirx $ assume(mod(size(q,1), 8) == 0)
  ! dirx $ assume(mod(size(q,2), 8) == 0)
...
!+ &
!(  w0(2:i1,2:j1,k+1) * ( rhobf(k+1)/rhobf(k) * p(2:i1,2:j1,k+1) +  p(i,j,k)) &
!  -w0(2:i1,2:j1,k)   * ( rhobf(k-1)/rhobf(k) * p(2:i1,2:j1,k-1) +  p(i,j,k)) &
!) / (2. * dzf(k)) &
!)
